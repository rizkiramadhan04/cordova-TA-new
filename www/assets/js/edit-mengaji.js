...
if (firstCon == "online") {
  if (status_user == "Guru") {
    ...
    $.ajax({
      ...
      type: "GET",
      url: conn + `/get-data-pencatatan-id/${data_id}`,
      dataType: "json",
      timeout: timeout,
      // data: data,
    })
      .done(function (values) {
        ...
        // jenis kitab
        var jenis_kitab = results.jenis_kitab;
        window.localStorage.removeItem("jenis_kitab");
        window.localStorage.setItem("jenis_kitab", jenis_kitab);
...
$(document).ready(function () {
  ...
  var jenis_kitab_loc = window.localStorage.getItem("jenis_kitab");
  if (jenis_kitab_loc == "Al Quran") {
    $("#iqro").hide();
    $("#surah").show();
    $("#ayat").show();
    $("#juz_wrap").show();
...
function postData() {
  ...
  $.ajax({
    ...
    type: "PUT",
    url: conn + `/update-data-pencatatan/${data_id}`,
    dataType: "json",
    timeout: timeout,
    data: data,
  })
